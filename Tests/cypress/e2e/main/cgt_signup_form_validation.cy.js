...
describe('Validate signup form of Caregiver tracker', () => {
  ...
  it('verify whether an error message is displayed when fields are left blank', () => {
    ...
    cy.get('#signUpPasswordError').should('have.text', 'Password field is required')
    cy.get('input[name="signUpRepeatPassword"]').focus().blur()
    cy.get('#signUpRepeatPasswordError').should('have.text', 'Repeat Password field is required')
    cy.get('input[name="signUpFullName"]').focus().blur()
    cy.get('#signUpFullNameError').should('have.text', 'Full Name field is required')
    cy.get('input[name="signUpAddVerify"]').focus().blur()
    cy.get('#signUpAddVerifyError').should('have.text', 'Adding numbers is required')
  ...
  it('verify whether an error message is displayed when invalid entries are entered', () => {
    ...
    cy.get('#signUpEmailError').should('have.text', 'Please enter a valid email')
    cy.get('input[name="signUpPassword"]').type('abcdefghi')
    cy.get('#signUpPasswordError').should('have.text', 'Password should have atleast one uppercase, lowercase, number and special character')
    ...
    cy.get('#signUpRepeatPasswordError').should('have.text', 'Password and Repeat Password should be the same')
    cy.get('input[name="signUpFullName"]').type('Name 123')
    cy.get('#signUpFullNameError').should('have.text', 'Full name cannot have number or special characters')
    cy.get('input[name="signUpFullName"]').clear().type('Full_Name')
    cy.get('#signUpFullNameError').should('have.text', 'Full name cannot have number or special characters')
    cy.get('label[for="signUpAddVerify"]').invoke('text').then(($addElem) => {
    ...
    cy.get('@sumSideVar').then(sumSide => {
      cy.get('input[name="signUpAddVerify"]').clear().type(sumSide-1)
      cy.get('#signUpAddVerifyError').should('have.text', 'Not properly added!')
    })    
  })
